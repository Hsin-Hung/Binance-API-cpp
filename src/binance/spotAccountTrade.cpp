...
void SpotAccountTrade::CancelReplace(CancelReplaceParams params, json &result)
{
    ...
    if (curl)
    {
        ...
        query_params.add_new_query("cancelReplaceMode", get_CancelReplaceMode(params.mode));
        query_params.add_new_query("timeInForce", get_TimeInForce(params.timeInForce));
        query_params.add_new_query("quantity", params.quantity);
        query_params.add_new_query("quoteOrderQty", params.quoteOrderQty);
        query_params.add_new_query("price", params.price);
        query_params.add_new_query("cancelNewClientOrderId", params.cancelNewClientOrderId);
        query_params.add_new_query("cancelOrigClientOrderId", params.cancelOrigClientOrderId);
        query_params.add_new_query("cancelOrderId", params.cancelOrderId);
        query_params.add_new_query("newClientOrderId", params.newClientOrderId);
        query_params.add_new_query("stopPrice", params.stopPrice);
        query_params.add_new_query("trailingDelta", params.trailingDelta);
        query_params.add_new_query("icebergQty", params.icebergQty);
        ...
        generate_HMAC_SHA256_sig(secret_key, query_params.to_str(), sig);
        query_params.add_signature(sig);
        std::string url = endpoint + "/api/v3/order/cancelReplace?" + query_params.to_str();
        std::vector<Header> headers;
